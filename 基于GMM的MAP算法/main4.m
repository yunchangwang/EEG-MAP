...
%-----------------------这里先导入第一个人的数据测试一下--------------------%
    ...
    filename=strcat('D:\基于GMM的EM算法\2008BCI数据\A0XT_V1\subject',str_k,'_V1.mat');
    ...
    for k=1:K
         ...
         idtmp = find(signal_filter_data.trials.data(:,23)==k);
    ...
    for a=1:10
        ...
        %训练
        [loglik_all,Pix_all,W,Mu,Sigma ] = GMM_MAP_train(trainData,K,0.35);
        %记录权重
        all_W{p,a}=W;
        all_loglik{p,a}=loglik_all;
        all_Pix{p,a}=Pix_all;
        %测试
        [LABELS_TEST] = GMM_MAP_test( testData,Mu,Sigma,K );
        %计算kapa系数
